...
This entrypoint:
- ensures the launchd-backed Live RAG service is installed and running,
- waits for the local webhook `/health`,
- resumes `sync --follow` from the last ingested `log_id`,
- queries `/retrieve` and returns message hits with conversation context.
